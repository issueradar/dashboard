...
export default function Post() {
  ...
  const { data: post, isValidating } = useSWR<WithProjectPost>(
    ...
    try {
      const response = await fetch(`/api/post`, {
        ...
        body: JSON.stringify({
          ...
          content: data.content,
          published: true,
          subdomain: post?.project?.subdomain,
          customDomain: post?.project?.customDomain,
          slug: post?.slug,
        }),
      });
      ...
      if (response.ok) {
        ...
        router.push(
          `https://${post?.project?.subdomain}.vercel.pub/${post?.slug}`,
        );
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      <Layout projectId={post?.project?.id}>
